# ADR-0098: Jail base archives and the empty-root safety check
...
- **`JailDefinition` gains `base_archive_name`** (`api/rpc/manager.proto`,
  `api/internalpb/state.proto`, field 10) - caller-set at `CreateJail`
...
This work was developed on a branch forked before ADR-0084 (jail base
templates via ZFS clone) existed, and originally numbered itself
ADR-0083 - a genuine collision with the already-shipped ADR-0083
(Mutually-authorized Colony join), discovered only at merge time.
Renumbered to ADR-0098 throughout (this file, its own field number
above, and every source comment referencing it) rather than editing
history on either side.

Two consequences of merging two independently-developed features that
both extend `JailDefinition`:

- **The "ZFS clone-from-template" alternative this ADR considered and
  rejected above was, unknown to this branch, built anyway** - as
  `base_template` (ADR-0084), on `main` while this branch sat stale.
  Both now coexist as genuinely different, non-redundant options
  (`base_template` clones a pre-populated operator-managed dataset;
  `base_archive_name` extracts an uploaded archive into a plain one),
  matching how VMs already offer more than one provisioning path
  (`base_image_name`, `clone_from_snapshot`). `ensureJail` now refuses
  a jail that names both, rather than letting one silently win.
- **Three more tests needed the same placeholder-root fix this ADR's
  own "Consequences" section already describes for its own three** -
  ADR-0084's own template tests (`TestReconciler_RunOnce_ClonesJailFromBaseTemplate`,
  `_FetchesMissingJailTemplateFromPeerBeforeCloning`,
  `_BaseTemplateNeverReClonesExistingDataset`) used a bare `t.TempDir()`
  mountpoint too, and the fake ZFS manager's `Clone` doesn't actually
  write anything to disk the way a real `zfs clone` would - so the new
  unconditional empty-root check would have broken them for a reason
  unrelated to what they actually test. Fixed the same way, with
  `writePlaceholderJailRoot`. (One of the three, the peer-fetch test,
  was missed on the first pass and only caught by actually running
  `go test` after resolving the textual conflicts - a reminder that a
  clean merge and a correct merge aren't the same thing.)
